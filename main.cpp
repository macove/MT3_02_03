...
bool IsCollision(const Segment segment, const Plane& plane) {
	...
	Vector3 segmentOrigin = segment.origin;
	Vector3 segmentDirection = segment.diff;

	float numerator = planeDistance - Dot(segmentOrigin, n);
	float denominator = Dot(segmentDirection, n);
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	while (Novice::ProcessMessage() == 0) {
		...
		ImGui::Begin("Window");
		ImGui::DragFloat3("cameraPositionA", &cameraPosition.x, 0.03f);
